...
def compute_ntl_loss(num_ids, num_vals, token_id_to_val, last_hidden, labels, lm_head):
    ...
    if num_pos_mask.any():
        ...
        logits_num = h_num @ partial_lm_head.T
        p_num = torch.softmax(logits_num, dim=-1)

        # E[v] for each position
        exp_val = (p_num * num_vals).sum(dim=-1)  # (K,)

        ntl_loss = torch.nn.functional.mse_loss(exp_val, tgt_val, reduction="mean")
    else:
